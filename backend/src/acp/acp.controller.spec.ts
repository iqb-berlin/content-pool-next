...
describe('AcpController', () => {
  ...
  beforeEach(() => {
    acpService = {
      ...
      updateIndex: jest.fn().mockResolvedValue({ packageId: 'pkg-1', version: '1.0.0' }),
      importIndex: jest.fn().mockResolvedValue({ packageId: 'pkg-1', version: '1.1.0' }),
      deleteIndex: jest.fn().mockResolvedValue({ packageId: 'pkg-1', version: '0.5.0' }),
      getAssignableUsers: jest.fn().mockResolvedValue([{ id: 'u-1' }]),
      getRoles: jest.fn().mockResolvedValue([{ userId: 'u-1', role: 'READ_ONLY' }]),
      assignRole: jest.fn().mockResolvedValue({ userId: 'u-1', role: 'READ_ONLY' }),
      removeRole: jest.fn().mockResolvedValue(undefined),
      getAccessConfig: jest.fn().mockResolvedValue({ accessModel: 'PUBLIC' }),
      updateAccessConfig: jest.fn().mockResolvedValue({ accessModel: 'PUBLIC' }),
      uploadCredentials: jest.fn().mockResolvedValue({ added: 1, updated: 2, skipped: 3 }),
      ...
      updateCredential: jest.fn().mockResolvedValue({ id: 'cred-1', username: 'new-name' }),
      updateMetadataColumns: jest.fn().mockResolvedValue({ metadataColumns: { visible: ['col1'] } }),
    };
    ...
    itemExplorerStateService = {
      ...
      discardDraft: jest.fn().mockResolvedValue({ status: 'CLEAN', version: 4 }),
  ...
  it('findAll returns all ACPs for app admins', async () => {
    const result = await controller.findAll({ user: { isAppAdmin: true, sub: 'u-1' } });
  ...
  it('findAll returns user-scoped ACPs for non-admins', async () => {
    const result = await controller.findAll({ user: { isAppAdmin: false, sub: 'u-2' } });
  ...
  it('delegates basic ACP CRUD and index methods', async () => {
    ...
    await expect(controller.create({ packageId: 'pkg-1', name: 'ACP 1' } as any)).resolves.toEqual({
      id: 'acp-1',
    });
    await expect(controller.update('acp-1', { name: 'Updated' } as any)).resolves.toEqual({
      id: 'acp-1',
      name: 'Updated',
    });
    await expect(controller.delete('acp-1')).resolves.toBeUndefined();
    await expect(controller.getIndex('acp-1')).resolves.toEqual({ packageId: 'pkg-1' });
    await expect(controller.updateIndex('acp-1', { assessmentParts: [] })).resolves.toEqual({
      packageId: 'pkg-1',
      version: '1.0.0',
    });
    await expect(controller.importIndex('acp-1', { assessmentParts: [] })).resolves.toEqual({
      packageId: 'pkg-1',
      version: '1.1.0',
    });
  ...
  it('returns assignable users and ACP roles', async () => {
    await expect(controller.getAssignableUsers('acp-1')).resolves.toEqual([{ id: 'u-1' }]);
  ...
  it('forbids removal of ACP_MANAGER role for non-admin users', async () => {
    acpService.getRoles.mockResolvedValueOnce([{ userId: 'u-2', role: 'ACP_MANAGER' }]);
  ...
  it('allows non-admin removal of READ_ONLY roles', async () => {
    acpService.getRoles.mockResolvedValueOnce([{ userId: 'u-2', role: 'READ_ONLY' }]);

    await controller.removeRole('acp-1', 'u-2', { user: { isAppAdmin: false } });
  ...
  it('handles access config and credential endpoints', async () => {
    ...
    const uploadResult = await controller.uploadCredentials(
      'acp-1',
      'append',
      { credentials: [{ username: 'u', password: 'p' }] } as any,
    );
    ...
    await expect(controller.getCredentials('acp-1')).resolves.toEqual([{ id: 'cred-1' }]);
    await expect(controller.deleteCredential('acp-1', 'cred-1')).resolves.toEqual({
      message: 'Credential deleted successfully',
    });
    await expect(
      controller.createCredential('acp-1', { username: 'new-user', password: 'pw' } as any),
    ).resolves.toEqual({ id: 'cred-1' });
    await expect(
      controller.updateCredential('acp-1', 'cred-1', { username: 'new-name' } as any),
  ...
  it('patches item explorer draft', async () => {
    ...
    const dto = { patch: { ui: { filterText: 'abc' } }, changeType: 'UI_UPDATE', baseVersion: 5 } as any;

    const result = await controller.patchItemExplorerDraft('acp-1', dto, req);

    expect(itemExplorerStateService.resolveActor).toHaveBeenCalledWith(req.user, 'acp-1');
    expect(itemExplorerStateService.patchDraft).toHaveBeenCalledWith('acp-1', dto.patch, {
  ...
  it('patches item explorer draft with empty patch fallback', async () => {
    ...
    expect(itemExplorerStateService.patchDraft).toHaveBeenCalledWith('acp-1', {}, expect.any(Object));
  });
  ...
  it('saves and discards item explorer drafts', async () => {
    ...
    await expect(
      controller.discardItemExplorerDraft('acp-1', { baseVersion: 9 } as any, req),
  ...
  it('lists item explorer changes with parsed and fallback limit', async () => {
    ...
    expect(itemExplorerStateService.listChanges).toHaveBeenCalledWith('acp-1', 25);

    await controller.getItemExplorerChanges('acp-1', 'not-a-number');
    expect(itemExplorerStateService.listChanges).toHaveBeenCalledWith('acp-1', 100);
  });
});

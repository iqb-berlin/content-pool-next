...
describe('ServerApiService', () => {
  ...
  it('lists and reads transfer files and throws when file is missing', async () => {
    ...
    await expect(service.getFile('acp-1', 'missing')).rejects.toThrow(NotFoundException);
  });

  it('throws when file operations are requested for a missing ACP', async () => {
    acpRepository.findOne.mockResolvedValue(null);

    await expect(service.listFiles('missing-acp')).rejects.toThrow(NotFoundException);
    await expect(service.downloadFile('missing-acp', 'file-1')).rejects.toThrow(NotFoundException);
  });
  ...
  it('downloads files through FilesService when ACP exists', async () => {
    ...
    filesService.downloadForAcp.mockResolvedValue({ buffer: Buffer.from('x'), file: { id: 'f1' } });
  ...
  it('rejects upload requests without files or with invalid conflictStrategy', async () => {
    ...
    await expect(service.uploadFiles('acp-1', [])).rejects.toThrow(BadRequestException);
    await expect(
      service.uploadFiles('acp-1', [{ originalname: 'f.xml' } as any], 'invalid'),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it('validates replacement file list details before processing', async () => {
    await expect(service.replaceCodingSchemeFiles('acp-1', [])).rejects.toThrow(BadRequestException);
    ...
    await expect(
      service.replaceCodingSchemeFiles('acp-1', [{ originalname: '   ' } as any]),
    ).rejects.toThrow(BadRequestException);

    await expect(
      service.replaceCodingSchemeFiles(
        'acp-1',
        [{ originalname: 'unit-1.vocs' } as any, { originalname: 'UNIT-1.VOCS' } as any],
      ),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it('validates receiveAcp payload shape and conflictStrategy', async () => {
    ...
    await expect(
      service.receiveAcp({ packageId: 'pkg-1', name: ' ', acpIndex: {} } as any),
    ).rejects.toThrow(BadRequestException);
    await expect(
      service.receiveAcp({ packageId: 'pkg-1', name: 'x', acpIndex: [] as any } as any),
    ).rejects.toThrow(BadRequestException);
    await expect(
      service.receiveAcp({ packageId: 'pkg-1', name: 'x', acpIndex: {} }, 'invalid'),
    ).rejects.toThrow(BadRequestException);
  });

...
describe('CommentsController', () => {
  ...
  beforeEach(() => {
    commentsService = {
      ...
      exportCommentsByCredential: jest.fn().mockResolvedValue([{ id: 'c-export-cred' }]),
      exportCommentsXlsx: jest.fn().mockResolvedValue(Buffer.from('xlsx-all')),
      exportCommentsXlsxByCredential: jest.fn().mockResolvedValue(Buffer.from('xlsx-cred')),
    };
  ...
  it('rejects all-comments access for non-managers', async () => {
    ...
    await expect(controller.findAll('acp-1', req)).rejects.toThrow(ForbiddenException);
  });
  ...
  it('returns mine for credential users', async () => {
    ...
    expect(commentsService.findByCredential).toHaveBeenCalledWith('acp-1', 'cred-user');
  ...
  it('creates comment directly for managers', async () => {
    const req = { user: { type: 'user', sub: 'u-1', isAppAdmin: true }, acpAccessLevel: 'PUBLIC' };
    const dto = {
  ...
  it('rejects create for non-managers when commenting is disabled', async () => {
    ...
    const req = { user: { type: 'credential', username: 'cred' }, acpAccessLevel: 'PUBLIC' };
    const dto = {
  ...
  it('rejects delete all for non-managers', async () => {
    ...
    await expect(controller.deleteAll('acp-1', req)).rejects.toThrow(ForbiddenException);
  });

  it('exports comments for manager users', async () => {
    const req = { user: { isAppAdmin: false, type: 'user', sub: 'u-1' }, acpAccessLevel: 'MANAGER' };
  ...
  it('exports comments for credential users with credential filter', async () => {
    const req = { user: { isAppAdmin: false, type: 'credential', username: 'cred-user' }, acpAccessLevel: 'PUBLIC' };
  ...
  it('exports comments for normal users with user filter', async () => {
    ...
    expect(commentsService.exportComments).toHaveBeenCalledWith('acp-1', 'u-42');
  });
  ...
  it('exports XLSX for credential users with username fallback', async () => {
    ...
    expect(commentsService.exportCommentsXlsxByCredential).toHaveBeenCalledWith('acp-1', '');
    expect(res.setHeader).toHaveBeenCalledWith(
      'Content-Disposition',
  ...
  it('exports XLSX for normal users with username fallback', async () => {
    ...
    expect(commentsService.exportCommentsXlsx).toHaveBeenCalledWith('acp-1', 'u-2');
    expect(res.setHeader).toHaveBeenCalledWith(
      'Content-Disposition',
      'attachment; filename="comments-acp-1-mine.xlsx"',
    );

...
describe('CommentsService', () => {
  ...
  beforeEach(() => {
    commentRepository = {
      find: jest.fn(),
      create: jest.fn().mockImplementation((value) => ({ id: 'c-1', ...value })),
    ...
    service = new CommentsService(commentRepository as any, accessConfigRepository as any);
  });
  ...
  it('queries comments by ACP, user and credential', async () => {
    ...
    await expect(service.findByUser('acp-1', 'u-1')).resolves.toEqual([{ id: 'c-1' }]);
    await expect(service.findByCredential('acp-1', 'reader')).resolves.toEqual([{ id: 'c-1' }]);
  ...
  it('exports comments by credential and prefers credential author', async () => {
    ...
    const result = await service.exportCommentsByCredential('acp-1', 'reader-1');
  ...
  it('builds XLSX buffers for user and credential exports', async () => {
    ...
    const byCredential = await service.exportCommentsXlsxByCredential('acp-1', 'reader-1');
  ...
  it('checks comment feature flags per target type', async () => {
    ...
    await expect(service.isCommentingEnabled('acp-1', CommentTargetType.ITEM)).resolves.toBe(false);
    ...
    await expect(service.isCommentingEnabled('acp-1', CommentTargetType.ITEM)).resolves.toBe(true);
    ...
    await expect(service.isCommentingEnabled('acp-1', CommentTargetType.UNIT)).resolves.toBe(true);
    await expect(service.isCommentingEnabled('acp-1', CommentTargetType.ITEM)).resolves.toBe(false);
  });
});

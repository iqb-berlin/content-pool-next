...
describe('ItemsService', () => {
  ...
  beforeEach(() => {
    ...
    service = new ItemsService(acpRepository as any, accessConfigRepository as any, unitParserService as any);
  });
  ...
  it('returns item details by id and null for unknown items', async () => {
    jest.spyOn(service, 'getItems').mockResolvedValue([
      { itemId: 'item-1' } as any,
      { itemId: 'item-2' } as any,
    ]);

    await expect(service.getItem('acp-1', 'item-2')).resolves.toEqual({ itemId: 'item-2' });
  ...
  it('filters and sorts items', async () => {
    ...
    const filtered = await service.getFilteredItems('acp-1', 'u-', 'itemId', 'desc');
    expect(filtered.map((i) => i.itemId)).toEqual(['b-2', 'a-1']);

    const sortedAsc = await service.getFilteredItems('acp-1', undefined, 'name', 'asc');
  ...
  it('rejects empirical difficulty upload for missing ACP', async () => {
    ...
    await expect(
      service.uploadEmpiricalDifficulties('acp-1', Buffer.from('item;est\na;1')),
    ).rejects.toThrow(NotFoundException);
  });

  it('returns no updates when CSV has no data rows', async () => {
    acpRepository.findOne.mockResolvedValue({ id: 'acp-1', itemProperties: {} });
    unitParserService.getItemListFromFiles.mockResolvedValue({ items: [] });

    const result = await service.uploadEmpiricalDifficulties('acp-1', Buffer.from('item;est'));
  ...
  it('validates required CSV headers', async () => {
    ...
    await expect(
      service.uploadEmpiricalDifficulties('acp-1', Buffer.from('wrong;headers\na;1')),
  ...
  it('updates matched empirical difficulties and collects unmatched rows', async () => {
    ...
    const result = await service.uploadEmpiricalDifficulties('acp-1', Buffer.from(csv));

    expect(result.updated).toBe(1);
    expect(result.failed).toEqual([{ csvRow: 'unknown-item', reason: 'Kein passendes Item gefunden' }]);
    expect(result.successes).toEqual([{ itemId: 'I-1', unitId: 'U-1', value: 0.75 }]);
  ...
  it('throws a bad request when one item appears multiple times in CSV', async () => {
    ...
    await expect(service.uploadEmpiricalDifficulties('acp-1', Buffer.from(csv))).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('rejects clearing empirical difficulties for missing ACP', async () => {
    ...
    await expect(service.clearEmpiricalDifficulties('acp-1')).rejects.toThrow(NotFoundException);
  });
  ...
  it('returns normalized item tag map and throws when ACP is missing', async () => {
    ...
    await expect(service.getItemTags('acp-1')).rejects.toThrow(NotFoundException);
  ...
  it('checks tag feature switch in ACP access config', async () => {
    accessConfigRepository.findOne.mockResolvedValueOnce({ featureConfig: { enableItemListTags: true } });
    await expect(service.canUseItemTags('acp-1')).resolves.toBe(true);

    accessConfigRepository.findOne.mockResolvedValueOnce({ featureConfig: { enableItemListTags: 0 } });
  ...
  it('saves normalized tags and replaces previous tag state', async () => {
    acpRepository.findOne.mockResolvedValueOnce(null);
    await expect(service.saveItemTags('acp-1', {})).rejects.toThrow(NotFoundException);

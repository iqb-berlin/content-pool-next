...
describe('UsersController', () => {
  ...
  beforeEach(() => {
    usersService = {
      ...
      update: jest.fn().mockResolvedValue({ id: 'u-1', displayName: 'Alice Updated' }),
  ...
  it('gets one user by id', async () => {
    await expect(controller.findOne('u-1')).resolves.toEqual({ id: 'u-1', username: 'alice' });
  ...
  it('creates users', async () => {
    ...
    await expect(controller.create(dto)).resolves.toEqual({ id: 'u-2', username: 'bob' });

...
describe("SyntacticValidator", () => {
  ...
  it("validates JSON content", () => {
    expect(
      validator.validate(
        { originalName: "data.json" } as any,
        Buffer.from('{"a":1}'),
      ).valid,
    ).toBe(true);

    const invalid = validator.validate(
      { originalName: "bad.json" } as any,
      Buffer.from('{"a":'),
    );
  ...
  it("validates XML heuristics", () => {
    const notXml = validator.validate(
      { originalName: "a.xml" } as any,
    ...
    const unbalanced = validator.validate(
      { originalName: "b.xml" } as any,
      Buffer.from("<root><child></root>"),
    );
  ...
  it("validates CSV column consistency", () => {
    const csv = "item;est\nA;1\nB;2;extra\n";
    const result = validator.validate(
      { originalName: "scores.csv" } as any,
      Buffer.from(csv),
    );

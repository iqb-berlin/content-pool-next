...
export interface SyntacticValidationResult {
  valid: boolean;
  issues: { severity: 'error' | 'warning' | 'info'; message: string; field?: string }[];
}
...
export class SyntacticValidator {
  ...
  validate(file: AcpFile, buffer: Buffer): SyntacticValidationResult {
    ...
    switch (ext) {
      ...
      default:
        issues.push({ severity: 'info', message: `No specific validator for .${ext} files` });
        return { valid: true, issues };
    }
  }

  private validateJson(buffer: Buffer, issues: SyntacticValidationResult['issues']): SyntacticValidationResult {
    try {
  ...
  private validateXml(buffer: Buffer, issues: SyntacticValidationResult['issues']): SyntacticValidationResult {
    const content = buffer.toString('utf-8').trim();

    // Basic XML structure check
    if (!content.startsWith('<?xml') && !content.startsWith('<')) {
      issues.push({ severity: 'error', message: 'File does not appear to be valid XML' });
      return { valid: false, issues };
    }
    ...
    if (openTags !== closeTags + selfClosing) {
      issues.push({ severity: 'warning', message: 'XML tags may not be balanced' });
    }

    return { valid: true, issues };
  }

  private validateCsv(buffer: Buffer, issues: SyntacticValidationResult['issues']): SyntacticValidationResult {
    const content = buffer.toString('utf-8');
    const lines = content.split('\n').filter(l => l.trim());

    if (lines.length === 0) {
      issues.push({ severity: 'warning', message: 'CSV file has no data rows' });
      return { valid: true, issues };
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (cols !== headerCols) {
        issues.push({ severity: 'warning', message: `Row ${i + 1} has ${cols} columns, expected ${headerCols}` });
      }
    }
